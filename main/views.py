...
def admin_memo_view(request):
    ...
    if request.method == 'POST':
        ...
        if action in ('create', 'update'):
            ...
            instruction.category = request.POST.get('category', 'almauni')
            ...
            CATEGORY_GROUPS = {
                'platonus': ['pps'],
                'helpdesk': ['it'],
                'documentolog': ['aup', 'buh', 'hr'],
            }

            group_codes = CATEGORY_GROUPS.get(instruction.category, [])
            allowed_positions = Position.objects.filter(group__in=group_codes)
            instruction.allowed_positions.set(allowed_positions)

            messages.success(request, 'Инструкция сохранена')
            return redirect('main:admin_memo')

        elif action == 'delete' and instruction_id:
